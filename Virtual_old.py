...
class Controller(udi_interface.Node):
    ...
    def parameterHandler(self, params):
            ...
            if a == "parseDelay":
                ...
                if val == "switch":
                    if not self.poly.getNode(key):
                        ...
                        self.poly.addNode(
                            VirtualSwitch(self.poly, self.address, key, _name)
                        )
                elif val == "temperature":
                    _name = str(val) + " " + str(key)
                    if not self.poly.getNode(key):
                        self.poly.addNode(
                            VirtualTemp(self.poly, self.address, key, _name)
                        )
                elif val == "temperaturec" or val == "temperaturecr":
                    if not self.poly.getNode(key):
                        _name = str(val) + " " + str(key)
                        self.poly.addNode(
                            VirtualTempC(self.poly, self.address, key, _name)
                        )
                elif val == "generic" or val == "dimmer":
                    if not self.poly.getNode(key):
                        _name = str(val) + " " + str(key)
                        self.poly.addNode(
                            VirtualGeneric(self.poly, self.address, key, _name)
                        )
                else:
                    pass
            else:
        ...
        LOGGER.info(
            "Pull Delay set to %s seconds, Parse Delay set to %s seconds",
            self.pullDelay,
            self.parseDelay,
        )
...
class VirtualTemp(
    ...
    def pullFromID(self, command1, command2):
        if command2 == 0:
            ...
            try:
                if command1 == "/2/":
                    _newTemp = int(_value[7])
                if command1 == "/1/":
                    _newTemp = int(_value[5])
            except Exception as e:
            ...
            if self.pullError:
                ...
                if (
                    self.tempVal == _testValRtoP
                    or self.tempVal == _testValCtoF
                    or self.tempVal == _testValRtoPandCtoF
                    or self.tempVal == _newTemp
                ):
                    pass
                else:
                    _lastUpdate = (
...
class VirtualTempC(
    ...
    def pullFromID(self, command1, command2):
        if command2 == 0:
            ...
            if self.pullError:
                ...
                if (
                    self.tempVal == _testValRtoP
                    or self.tempVal == _testValFtoC
                    or self.tempVal == _testValRtoPandFtoC
                    or self.tempVal == _newTemp
                ):
                    pass
                else:
    ...
    def avgHighLow(self):
        if (
            self.highTemp != -60 and self.lowTemp != 129
        ):  # make sure values have been set from startup
